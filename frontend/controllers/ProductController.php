...
class ProductController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        $this->navbar = '<a href="' . Yii::$app->urlManager->baseUrl . '/" target="_self">Home Page</a>';
    ...
    public function actionProduct($path, $id)
    {
        ...
        if ($parent->id == $catProduct->id) {
            $this->navbar .= ' / '.'<a href="' . Yii::$app->urlManager->createUrl(['product/product', 'path' => $path, 'id' => $id]) . '"> ' . $catProduct->name . '</a>';
        } else {

            if($parent->id!=$catProduct->parent){
                $parent2 = Catproduct::find()->where(['id'=>$catProduct->parent])->one();
                if(!is_null($parent2)){
                    $this->navbar .= ' / '.'<a href="' . Yii::$app->urlManager->createUrl(['product/product', 'path' => $parent2->url, 'id' => $parent2->id]) . '"> ' . $parent2->name . '</a>';
                }else{
                    $this->navbar .= ' / '.'<a href="' . Yii::$app->urlManager->createUrl(['product/product', 'path' => $parent->url, 'id' => $parent->id]) . '"> ' . $parent->name . '</a>';
                }
            }else{
                $this->navbar .= ' / '.'<a href="' . Yii::$app->urlManager->createUrl(['product/product', 'path' => $parent->url, 'id' => $parent->id]) . '"> ' . $parent->name . '</a>';
            }
    ...
    public function actionProduct2($path, $id)
    {
        $catProduct = Catproduct::find()->where('url = :u and id=:id', [':u' => $path, ':id' => $id])->one();
        $parent = Catproduct::getParent($catProduct->id);
        $anhem = Catproduct::getAllSubCat($catProduct->id);
        ...
        $valuecat=$catProduct;
        if ($valuecat->home == 1):
            $data = \common\models\Product::find()->where(['hot' => 1])->all();
        else:
            $sach = \common\models\Nhomsachtheoloai::findAll(['nhomid' => $valuecat->id]);
            $t = [];
            foreach ($sach as $valuesach) {
                $t[] = $valuesach->sachid;
            }
            $data = \common\models\Product::find()->where(['hot' => 1])->andWhere(['IN', 'id', $t])->all();
        endif;
        $products=$data;
        if (!is_null($catProduct)) {
    ...
    public function actionDetailproduct($path, $id)
    {
        ...
        if(is_null($product) || $product->hot==1){
            return $this->redirect(['site/error']);
        }
        /** @var Product $product */
        $valuecat=$product->catProduct;
        if(!(($valuecat->menustyle==0 && Yii::$app->user->isGuest)
            || ($valuecat->menustyle==1 && !Yii::$app->user->isGuest)
            || ($valuecat->menustyle==0 && !Yii::$app->user->isGuest)
            || ($valuecat->menustyle==2 && Yii::$app->user->isGuest))){
            return $this->render('dangkys');
        }
        ...
        $this->navbar .= ' / '.'<a href="' . Yii::$app->urlManager->createUrl(['product/product', 'path' => $product->catProduct->url, 'id' => $product->catProduct->id]) . '"> ' . $product->catProduct->name . '</a>';
    ...
    {
        $this->navbar .= '<li><a href="' . Yii::$app->urlManager->createUrl(['product/giohang']) . '"> Cart</a></li>';
        $giohang = [];
        $dongia = [];
        $phienban = [];
        $soluongchitiet = [];
    ...
    public function actionPayment()
    {
        ...
        if ($tongtien > 0) {
            $this->navbar .= '<li><a href="' . Yii::$app->urlManager->createUrl(['product/payment']) . '"> Checkout</a></li>';

            if ($request->isGet) {
                $model->gender = "Anh";
                $model->type = "Giao hàng tận nơi ở";
                $model->typethanhtoan = "Giao hàng thu tiền tận nhà (Nhận tiền thưởng sau)";
                ...
                foreach ($giohang as $index => $item):
                    ...
                    $dem+=$soluongchitiet[$index][$item['id']]; endforeach;
                if(!$checkall){
                ...
                return $this->render('payment/index', [
                    ...
                    'dongia' => $dongia,
                    'phienban' => $phienban,
                    'dem'=>$dem,'dataProvider' => $dataProvider
                ]);
            } else {
                ...
                if ($model->load($request->post()) && $model->validate()) {
                    ...
                    if ($model->hasErrors()) {
                        ...
                        if ($model->type == "Nhận tại cửa hàng") {
                            $bill->typethanhtoan = "Mua tại cửa hàng";
                        }
                        $bill->chuyendanhba=(Yii::$app->user->isGuest)?-1:Yii::$app->user->id;

                        if ($bill->save()) {
                            Yii::$app->session['currentbill']=$bill->id;
                            $bill->mangdtkhac = (Yii::$app->user->isGuest)?"Khách không đăng ký":"TULATO ".Yii::$app->user->id."U ".$bill->id;
                            $bill->update();
                            ...
                            Yii::$app->session["iddathang"]=$bill->id;

                            if(isset($_POST['chonsach'])){
                                $sachlist = explode("-",$_POST['chonsach']);
                                ...
                                foreach ($sachlist as $value){
                                    if(is_int((int)$value)&& (int)$value>0 && $counthack<$dem){
                                        ...
                                        if (is_null($bill) || is_null($sach)) {
                                            return $this->redirect(['product/done', 'type' => $model->typethanhtoan]);
                                        }
                            ...
                            if(Yii::$app->user->isGuest){
                                $userl=new LoginForm();
                                $userl->username= $_POST['sdt'];
                                $userl->password='123456';
                                $userl->login();
                                return $this->redirect(['product/done', 'type' => $model->typethanhtoan,'isGuest'=>true]);
                            }
    ...
    public function actionDone($type)
    {
        ...
        $listSach =[];
        $checkall=false;
        $model = Billmobile::findOne(Yii::$app->session['currentbill']);
        ...
        return $this->render('done', ['type' => $type,'bill'=>$model,'dem'=>$dem,'dataProvider' => $dataProvider]);
    }
    ...
    public function actionQuickview()
    {
        ...
        if (!is_null($parent)) {
            $this->navbar .= '<li><a href="' . Yii::$app->urlManager->createUrl(['product/product', 'path' => $parent->url, 'id' => $parent->id]) . '"> ' . $parent->name . '</a></li>';
        }
        $this->navbar .= '<li><a href="' . Yii::$app->urlManager->createUrl(['product/product', 'path' => $subp->url, 'id' => $subp->id]) . '"> ' . $subp->name . '</a></li>';

        $this->navbar .= '<li class="active"><a href="' . Yii::$app->urlManager->createUrl(['product/detailproduct', 'path' => $product->url, 'id' => $product->id]) . '"> ' . $product->name . '</a></li>';
