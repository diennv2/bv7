<?php
    $this->title="Cơ cấu tổ chức Bệnh viện Quân y 7";
?>
...
<style>
    #tree-container {
        ...
        position: relative;
        display: flex;
        justify-content: center; /* Căn giữa phần tử cha */
    }

    .orgchart {
        width: 100%; /* Đảm bảo cây tổ chức chiếm toàn bộ chiều rộng */
        height: 100%; /* Đảm bảo chiều cao của cây tổ chức không bị giới hạn */
...
<script>
    const orgData = {
        <?php $cctc = \common\models\Menu::find()->where(['position'=>1])->all();
            if(!empty($cctc)):
        ?>
        name: "<?=$cctc[0]->name?>",
        href: "<?=$cctc[0]->link?>",
        ...
        <?php if(count($cctc)>1):?>
        children: [
            <?php for ($i=1;$i<count($cctc);$i++):?>
            {
                name: "<?=$cctc[$i]->name?>",
                href: "<?=$cctc[$i]->link?>",
                displaycon: true,
                <?php $con = \common\models\Menu::find()
						->where(['parent' => $cctc[$i]->id, 'active' => 1])
						->orderBy(['ord' => SORT_ASC])
						->all();
                    if(!empty($con)):
                ?>
                children: [ // Các nốt con cấp 2 được đưa vào content sau
                    <?php foreach ($con as $valuecon):?>
                    { name: "<?=$valuecon->name?>", href: "<?=$valuecon->link?>", },
                    <?php endforeach;?>
                ]
                <?php endif;?>
            },
    ...
    $(function () {
        $('#tree-container').orgchart({
            ...
            'pan': true,
            'zoom': false, // Tắt zoom
            'direction': 't2b', // Hướng từ trái qua phải cho các nốt cấp 1
            'visibleLevel': 2, // Số cấp độ hiển thị
            'nodeContent': 'name', // Hiển thị tên cho các nốt
            'createNode': function ($node, data) {
                // Thêm các con vào phần content của mỗi node
                if (data.children && data.displaycon) {
                    let content = '<ul style="margin-bottom: 5px">';
                    data.children.forEach(function (child) {

                        content += `<li style="font-size: 14px; text-align: left;margin-bottom: 5px"><a target="_blank" href="${child.href}">- ${child.name}</a></li>`;
                    });
                    content += '</ul>';
                    $node.find('.content').html(content); // Thêm danh sách con vào phần content
                    delete data.children; // Xóa các con cấp 2 bên ngoài để tránh trùng lặp
                }else{
                    $node.find('.content').remove();
                }
                $node.find('.title').html(`<a target="_blank" href="${data.href}">${data.name}</a>`);
            }
        });
    });
</script>
