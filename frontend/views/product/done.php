        ...
        <div class="row  wrap_cart">
            <div class="col-md-12 ">
                ...
                <div class="alert alert-success padding15 margin-top-bot-10">
                    <div class="">
                        <p>Xin cám ơn.</p>
                        <p>Đơn hàng đã gửi lên hệ thống, Tulato sẽ liên hệ với bạn để xác nhận sớm nhất.</p>
                        <?php if ($type != "Chuyển khoản (Bạn sẽ được trả lời câu hỏi nội dung sách ngay)"): ?>
                            <p>Bạn đã chọn hình thức thanh toán: <b><?= $type ?></b>.</p>
                        <?php else: ?>
                            <p>Bạn đã chọn hình thức thanh toán: <b><?= $type ?></b>. Bạn vui lòng chuyển khoản với nội
                                dung chính xác "<strong>TULATO <?= Yii::$app->user->id ?>U <?= Yii::$app->session["iddathang"] ?></strong>" để việc
                                xác minh
                        ...
                        <div class="row" style="background: white;padding: 15px">
                            <div class="col-md-6 table-responsive">
                                <h3 style="text-transform: uppercase;font-weight: bolder">Thông tin đơn hàng</h3>
                                <table class="table table-striped table-hover table-bordered">
                                    ...
                                    foreach ($product['giohang'] as $index => $item):
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <?= $item['name'] ?><br>
                                                <span class="text-danger"><?= $product['phienban'][$index][$item['id']] ?>
                                            </td>
                                            </span>
                                            <td>
                                            ...
                                            <td>
                                                <?= number_format($product['dongia'][$index][$item['id']] * $product['soluongchitiet'][$index][$item['id']], 0, '', '.') . " " . $config['money_suffix'] ?>
                                                <br>
                                            </td>
                                        </tr>
                                        <?php $dem++; endforeach; ?>
                                    <tr>
                                        <td style="text-align: right;font-weight: bold" colspan="5">Tổng:</td>
                                        <td style="text-align: right;font-weight: bold"><?= number_format($product['tongtien'], 0, '', '.') . " " . $config['money_suffix'] ?>
                                            <br></td>
                                    </tr>
                                    <tr>
                                        <td style="text-align: right;font-weight: bold" colspan="5">VAT:</td>
                                        <td style="text-align: right;font-weight: bold"><?= number_format($model->vat, 0, '', '.') . " " . $config['money_suffix'] ?></td>
                                    </tr>
                                    <tr>
                                        <td style="text-align: right;font-weight: bold" colspan="5">Tổng sau VAT:</td>
                                        <td style="text-align: right;font-weight: bold"><?= number_format($model->tongsauvat, 0, '', '.') . " " . $config['money_suffix'] ?></td>
                                    </tr>
                                </table>
                            </div>
                            <div class="col-md-6 table-responsive">
                                <h3 style="text-transform: uppercase;font-weight: bolder">Quà tặng</h3>
                                <style>
                                    .table#ttrrsafa > tbody > tr > td, .table#ttrrsafa > tbody > tr > th, .table#ttrrsafa > tfoot > tr > td, .table#ttrrsafa > tfoot > tr > th, .table#ttrrsafa > thead > tr > td, .table#ttrrsafa > thead > tr > th {
                                        border-top: none;
                                    }
                                </style>
                                <table class="table" id="ttrrsafa">
                                    <tr>
                                        <td>

                                            <?php if ($bill->nhanquastatus == 0): ?>
                                                <a class="btn btn-success"
                                                   href="<?= Yii::$app->urlManager->createUrl(['site/nhanqua', 'id' => $bill->id]) ?>">
                                                    <i class="fa fa-gift"></i> Nhận quà</a>
                                            <?php else: ?>
                                                <table class="table">
                                                    <?php $listnhanquas = \common\models\Listnhanqua::findAll(['billid' => $bill->id]); ?>
                                                    <?php if (empty($listnhanquas)): ?>
                                                        <tr>
                                                            <td class="text-danger">Bạn không biết nên chọn sách nào
                                                                phải không? Tulato sẽ tặng bạn 1 cuốn sách ngẫu nhiên.
                                                            </td>
                                                        </tr>
                                                    <?php else: ?>
                                                        <?php foreach ($listnhanquas as $listnhanqua): ?>
                                                            ...
                                                            <?php if (is_null($product)): ?>
                                                                <tr>
                                                                    <td class="text-danger">Bạn không biết nên chọn sách
                                                                        nào phải không? Tulato sẽ tặng bạn 1 cuốn sách
                                                                        ngẫu nhiên.
                                                                    </td>
                                                                </tr>
                                                            <?php else: ?>
                                                                <tr>
                                                                    <td style="width: 150px"><img style="width: 150px"
                ...
                <?php if ($model->chuyendanhba != -1 || !Yii::$app->user->isGuest): ?>
                    <div class="alert alert-success padding15 margin-top-bot-10">
                        <div id="hotros" class="hidden" scroll="no"
                             style="position: fixed;background: #333333b8;top: 0;left: 0;width: 100vw;height: 100vh; z-index: 99">
                                ...
                                <div style="height:60%; width: calc(100% - 20px); margin: auto;border-radius: 4px;background: white;top:50%;transform: translateY(-50%);position: absolute">
                                    <div style="position: relative;height: 100%">
                                        ...
                                        <div class="hoi">
                                            ...
                                            <div class="resulttim">
                                                <a href="<?= $actual_link ?>&nhieunguoichon=1" class="findoptions">Tìm
                                                    cho tôi 10 cuốn sách nhiều người chọn nhất!</a>
                                                <?php $nhomsach = \common\models\Nhomsach::find()->all();
                                                foreach ($nhomsach as $nhomsachs):
                                                    ?>
                                                    <a href="<?= $actual_link ?>&filter=<?= $nhomsachs->id ?>"
                                                       class="findoptions">Tìm cho tôi <?= $nhomsachs->tennhom ?></a>
                                                <?php endforeach; ?>
                                                <a href="<?= $actual_link ?>&filter=popular" class="findoptions">Tôi
                                                    muốn tìm theo tên sách</a>
                        ...
                        <div class="" id="columns">
                            ...
                            <div class="page-content page-order">
                                ...
                                <div class="order-detail-content">
                                    ...
                                    <?php if ($bill->nhanquastatus == 0): ?>
                                        <p>Cám ơn bạn đã tin tưởng đặt hàng tại Tulato, dưới đây là phần quà của bạn,
                                            hãy click để chọn <strong><?= $dem ?> cuốn sách</strong> mà bạn
                                            thích nhất, sau đó ấn nút Chọn ở bên cạnh:
                                            <button class="btn btn-success" id="btn-chon-sach">Tôi chọn những cuốn sách
                                                này
                                            </button>
                                        </p>
                                    <?php if (isset($_GET['filter'])):
                                    $nhom = \common\models\Nhomsach::findOne($_GET['filter']);
                                    if (!is_null($nhom)): ?>
                                    <p style="margin: 10px 0;font-size: 18px" class="alert alert-warning">Bạn đang xem
                                        mục : <?= $nhom->tennhom ?> (Ngẫu nhiên) - <a class="btn btn-info" href="">Làm
                                            mới</a></p><?php endif;
                                    endif; ?>
                                        <div class="row">
